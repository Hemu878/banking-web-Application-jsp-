...
if(status==true)
{
	...
	if(rs1.next())
	{
		dataamount=amountn+rs1.getInt(4); 
		Connection con2=DriverManager.getConnection(url,user,pass);
		PreparedStatement pr2=con2.prepareStatement("update MyBank.Employee set Amount=? where AccNo='"+AccNo+"'");
		pr2.setInt(1,dataamount );
		pr2.executeUpdate();
		

		RequestDispatcher rd=request.getRequestDispatcher("diposite.jsp");
		rd.include(request, response);
		
		out.println("<html><style>h1{height: 50px;width: auto;background-color: aqua ;}</style><body><h1> Successfully "+amountn+" "+"RS/- Deposite...</h1></body></html>");
		out.println("<html><body><h1> Your Account Balances "+amountn+" "+"RS/- </h1></body></html>");
